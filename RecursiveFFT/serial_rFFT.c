...
#define PI acos(-1.0)  //to be precise. Proved to be more precise than Math.PI

//Defining a double complex type
typedef double complex dcom;

//Function used to calculate time in milliseconds
...
void display(dcom * array, int len)
{
	for(int i=0; i<len; i++)
	{
		printf("%g, %g\n", creal(array[i]), cimag(array[i])); //creal() prints the real component of the array while cimag() prints the imaginary component
	}
}
...
dcom * fastFourierTrans(dcom * inputArray, int len)
{
	...
	if(len == 1)
  {
	...
	//Here's where the input array is split and the recursive call occurs
	dcom * evenSubArray = fastFourierTrans(sub_array(inputArray, len, 0), len/2);
	dcom * oddSubArray = fastFourierTrans(sub_array(inputArray, len, 1), len/2);
	
	dcom * outputArray = malloc(len*sizeof(dcom));

  //stitching the even and odd sub arrays together following the algorithm 
